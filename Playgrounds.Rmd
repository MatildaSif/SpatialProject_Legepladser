...
muni_population <- muni_population %>%
  rename("municipality" = "NAME_2") %>% 
  dplyr::select(municipality, geometry, population) %>% 
  # make population column numeric
  mutate(population = as.numeric(gsub(" ", "", population)))

# plot
pal <- colorBin(palette = "OrRd", domain = muni_population$population, bins = 8)
...
leaflet(muni_population) %>%
  ...
  addPolygons(fillColor = ~pal(population),
              ...
              weight = 1,
              fillOpacity = 0.5,
              popup = ~paste(municipality, "<br>Population:", population)) %>%
  addLegend(pal = pal, values = ~population, opacity = 0.7,
            title = "Population",
            position = "bottomright") %>% 
  # add playgrounds
  addCircleMarkers(data = playgrounds, lng = ~longitude, lat = ~latitude, radius = 0.5,
