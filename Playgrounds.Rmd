...
```{r package-info, results='asis'}
# Define your packages
packages <- c("mapboxapi")
...
for (pkg in packages) {
  ...
  cat(sprintf("### %s (v%s)\n\n", pkg, version))
  cat(paste(capture.output(print(cit, style = "text")), collapse = "\n"))
  cat("\n\n---\n\n")
}
...
CALCULATE STATS for Cycling Iso
...
# Join covered area
coverage_data <- left_join(muni_data, intersections, by = "municipality")
...
coverage_data <- left_join(coverage_data, muni_income, by = "municipality") %>% 
    st_drop_geometry() %>% 
    dplyr::select(-contains("geometry"))


coverage_pivot <- coverage_data %>%
...
ggplot(coverage_data, aes(x = reorder(municipality, -uncovered_pct), y = uncovered_pct, fill = municipality)) +
...
# 3. Uncovered child density:  The number of children per square kilometer living in areas of a municipality that are not covered by  isochrones 
ggplot(coverage_data, aes(x = reorder(municipality, -uncovered_child_count), y = uncovered_child_count, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
# 4. Child Population
ggplot(coverage_data, aes(x = reorder(municipality, -child_population), y = child_population, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Child Population by Municipality",
       x = "Municipality", y = "Child Population") +
  theme_minimal() +
  theme(legend.position = "none")

# 5. Child Population in pct
ggplot(coverage_data, aes(x = reorder(municipality, -child_density), y = child_density, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
# 6. Proportion of Child Population Covered
ggplot(coverage_data, aes(x = reorder(municipality, -prop_children_covered), 
                          y = prop_children_covered)) +
  geom_col(fill = "skyblue") +
  labs(title = "Proportion of Child Population Covered by Cycling Isochrones",
       x = "Municipality", y = "Proportion Covered") +
  theme_minimal() +
  coord_flip()


# 7. Uncovered child count v. income
ggplot(coverage_data, aes(x = income, y = prop_children_covered, colour = municipality)) +
  geom_point( size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = "darkred") +
  labs(title = "Proportion of covered children vs Mean Income",
       x = "Mean Income", y = "Proportion of covered children") +
  theme_minimal()


# 8. Avg income by income quartile
ggplot(coverage_pivot, aes(x = factor(income_quantile), y = area, fill = coverage_type)) +
...
# 9. Covered area per municipality coloured by child density
ggplot(coverage_data, aes(x = municipality)) +
  geom_col(aes(y = (100- uncovered_pct), fill = child_density)) +
  labs(title = "% Covered Area per Municipality", y = "% covered Area",) +
  theme_minimal()

# 10. Uncovered child count vs income
ggplot(coverage_data, aes(x = income, y = uncovered_child_count, colour = municipality)) +
  geom_point(size = 3) +
...
```{r}
# 1. Uncovered area percentage
ggplot(walking_coverage_data, aes(x = reorder(municipality, -uncovered_pct), y = uncovered_pct, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
ggplot(walking_coverage_data, aes(x = reorder(municipality, -uncovered_child_count), y = uncovered_child_count, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
# 3. Child Population
# ggplot(walking_coverage_data, aes(x = reorder(municipality, -child_population), y = child_population, fill = municipality)) +
#   geom_bar(stat = "identity") +
#   coord_flip() +
#   labs(title = "Child Population by Municipality",
#        x = "Municipality", y = "Child Population") +
...
# 4. Child Density per municipality
ggplot(walking_coverage_data, aes(x = reorder(municipality, -child_density), y = child_density, fill = municipality)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Child Population Density by Municipality",
       x = "Municipality", y = "Child Density") +
...
# 5. Proportion of Child Population Covered
# ggplot(walking_coverage_data, aes(x = reorder(municipality, -prop_children_covered), 
#                           y = prop_children_covered)) +
#   geom_col(fill = "skyblue") +
#   labs(title = "Proportion of Child Population Covered by 5min Walking Isochrones",
#        x = "Municipality", y = "Proportion Covered") +
#   theme_minimal() +
#   coord_flip()
...
# 7. Uncovered child count v. income
# ggplot(walking_coverage_data_2, aes(x = income, y = prop_children_covered, colour = municipality)) +
#   geom_point( size = 3) +
#   geom_smooth(method = "lm", se = FALSE, color = "darkred") +
#   labs(title = "5min Walking Isochrone: Proportion of covered children vs Mean Income",
#        x = "Mean Income", y = "Proportion of covered children") +
#   theme_minimal()


# 8. Avg income by income quartile
ggplot(walking_coverage_data_2_pivot, aes(x = factor(income_quantile), y = area, fill = coverage_type)) +
  geom_bar(stat = "identity", position = "fill") +
...
# 9. Covered area per municipality coloured by child density
ggplot(walking_coverage_data, aes(x = municipality)) +
...
# 11. Uncovered child density vs income
  ggplot(walking_coverage_data_2, aes(x = income, y = uncovered_child_count, colour = municipality)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", color = "red") + 
  ylim(0,NA) +
...
coverage_stats <- coverage_stats %>%
  mutate(density_quantile = ntile(child_density, 4))
```
...
ggplot(playground_counts, aes(x = reorder(municipality, -n_playgrounds), y = n_playgrounds, fill = municipality)) +
  geom_bar(stat = "identity") +
...
ggplot(coverage_stats, aes(x = reorder(municipality, -playgrounds_per_km2), y = playgrounds_per_km2, fill = municipality)) +
  geom_bar(stat = "identity") +
...
ggplot(coverage_stats, aes(x = reorder(municipality, -playgrounds_per_1000_children), y = playgrounds_per_1000_children, fill = municipality)) +
  geom_bar(stat = "identity") +
...
ggplot(coverage_stats, aes(x = income_quantile, y = uncovered_child_count, fill = municipality)) +
  geom_bar(stat = "identity") +
  labs(title = "Estimated Proportion of Uncovered Children by Income Quantile",
       x = "Income Quantile", y = "Estimated proportion of uncovered children")

# density quantiles
ggplot(coverage_stats, aes(x = density_quantile, y = uncovered_child_count, fill = municipality)) +
  geom_bar(stat = "identity") +
  labs(title = "Estimated Proportion of Uncovered Children by Child Density Quantile",
       x = "Child Density Quantile", y = "Estimated Proportion of Uncovered Children")



```
Percentage cover increase from walking to cycling isochrone
```{r}
# select covered_pct
cycling_covered <- coverage_data %>%
...
# Plot
ggplot(joined_covered, aes(x = reorder(municipality, -pct_increase), y = pct_increase)) +
...
# plotpercentage increase along with walking_covered_pct and for cycling
# Plot with legend
ggplot(long_covered, aes(x = reorder(municipality, -covered_pct), y = covered_pct, fill = mode)) +
...
# plt number of children walking v. cycling
ggplot(uncovered_long, aes(x = reorder(municipality, -uncovered_child_count), y = uncovered_child_count, fill = mode)) +
...
# Assuming you have a dataframe `joined_covered` with these variables:
# municipality, child_density, uncovered_prop, income_quartile

ggplot(coverage_stats, aes(x = child_density, y = uncovered_child_count)) +
  geom_point(aes(color = as.factor(income_quantile)), size = 4, alpha = 0.7) +
...
ggplot(coverage_stats, aes(x = as.factor(income_quantile), y = uncovered_child_count)) +
  geom_point(aes(size = child_density, color = as.factor(income_quantile)), alpha = 1) +
